...
const provider=process.env.CLOUD_BOARD_TEST_PROVIDER || 'google.com';
assert.ok(['google.com','apple.com'].includes(provider));
const encode=value=>Buffer.from(JSON.stringify(value)).toString('base64url');
const token=`${encode({alg:'none',typ:'JWT'})}.${encode({sub:'deletion-test-google',email:'delete-test@example.invalid',email_verified:true,name:'Emulator only',iss:provider==='apple.com'?'https://appleid.apple.com':'https://accounts.google.com',aud:'demo-client'})}.`;
const response=await fetch(`http://${process.env.FIREBASE_AUTH_EMULATOR_HOST}/identitytoolkit.googleapis.com/v1/accounts:signInWithIdp?key=demo-key`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({postBody:`id_token=${token}&providerId=${provider}`,requestUri:'http://localhost',returnSecureToken:true})});
