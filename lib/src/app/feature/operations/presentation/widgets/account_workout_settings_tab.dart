...
class _SettingsForm extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final draft = useState(
      initial
          .applyTo(
            Workout.empty(
              'account-settings',
              WorkoutAuthor(id: ownerId, displayName: '', photoUrl: null),
            ),
          )
          .copyWith(name: '운동 제목'),
    );
    ...
    final current = WorkoutPreferences.fromWorkout(
      draft.value.copyWith(brandL: brandL.text, brandR: brandR.text),
    );
                ...
                FilledButton.icon(
                  onPressed: action.isLoading
                      ...
                      : () async {
                          ...
                          if (success) baseline.value = current;
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                success ? '계정 공통 설정을 저장했습니다. 다음 수업부터 적용됩니다.' : '저장하지 못했습니다. 연결과 로그인 상태를 확인하고 다시 시도해 주세요.',
                              ),
                            ),
                          );
                        },
