...
void main() {
  ...
  for (final size in [
    ...
    testWidgets(
      'four tabs preserve draft, history and color validation at $size',
      (tester) async {
        ...
        await tester.tap(find.byTooltip('실행 취소'));
        await tester.pumpAndSettle();
        expect(container.read(provider).module.appearance.showTitle, isTrue);
        await tester.tap(find.byTooltip('다시 실행'));
        await tester.pumpAndSettle();
        await tab('타이머');
        final color = find.byKey(const ValueKey('color-swatch-운동 게이지 색상'));
        await reveal(color);
